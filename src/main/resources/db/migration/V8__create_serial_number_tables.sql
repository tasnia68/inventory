...
CREATE TABLE IF NOT EXISTS serial_numbers (
    ...
    CONSTRAINT uq_serial_product UNIQUE (serial_number, product_variant_id, tenant_id)
);

-- 3. Create Stock Movement Serial Numbers Table
CREATE TABLE IF NOT EXISTS stock_movement_serial_numbers (
    id UUID PRIMARY KEY,
    ...
    updated_by VARCHAR(255),
    tenant_id VARCHAR(255) NOT NULL,
    CONSTRAINT fk_smsn_movement FOREIGN KEY (stock_movement_id) REFERENCES stock_movements (id),
    CONSTRAINT fk_smsn_serial FOREIGN KEY (serial_number_id) REFERENCES serial_numbers (id)
);

...
@Slf4j
@RestController
@RequestMapping("/api/webhooks/shopify/{tenantId}")
@RequiredArgsConstructor
public class ShopifyWebhookController {

    private static final String SHOPIFY_SIGNATURE_HEADER = "X-Shopify-Hmac-Sha256";
    private static final String SHOPIFY_TOPIC_HEADER = "X-Shopify-Topic";
    private static final String SECRET_KEY = "shopify.webhook_secret";
    ...
    public ResponseEntity<ApiResponse<ExternalOrderIngestionService.IngestionResult>> orders(
            ...
            @RequestHeader(value = SHOPIFY_SIGNATURE_HEADER, required = false) String signature,
            @RequestHeader(value = SHOPIFY_TOPIC_HEADER, required = false) String topic,
            @RequestBody byte[] rawBody) {
        String secret = tenantSettingService.findSetting(SECRET_KEY).map(s -> s.getValue()).orElse(null);
        if (secret == null || secret.isBlank()) {
            log.warn("Shopify webhook received but tenant secret {} not configured", SECRET_KEY);
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(ApiResponse.<ExternalOrderIngestionService.IngestionResult>builder()
                            .success(false).status(503).message("Shopify webhook secret not configured").build());
        }
        if (!signatureVerifier.verifyHmacSha256(rawBody, signature, secret, WebhookSignatureVerifier.Encoding.BASE64)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(ApiResponse.<ExternalOrderIngestionService.IngestionResult>builder()
                            .success(false).status(401).message("Invalid Shopify webhook signature").build());
        }
